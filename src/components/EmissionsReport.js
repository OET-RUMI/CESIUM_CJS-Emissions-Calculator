...
const EmissionsReport = ({ emissionsData, onClose }) => {
  ...
  const defaultData = {
    co2e: 0,
    hub_equipment_co2e: 0,
    vehicle_operation_co2e: 0,
    vehicle_energy_provision_co2e: 0,
    distance_km: 0,
    cargo_tonnes: 0,
    co2e_calculation_method: 'unknown',
    route: []
  };
  
  // Merge with defaults to handle missing properties
  emissionsData = { ...defaultData, ...emissionsData };
  ...
  return (
    <Card className="fixed top-4 right-4 z-10 modern-card" style={{ width: "550px", height: "calc(100vh - 8rem)" }}>
      <CardHeader className="pb-2 pt-3 px-4 border-b border-gray-600">
        <CardTitle className="flex justify-between items-center text-lg text-white">
          ...
          <Button 
            ...
            size="sm" 
            onClick={onClose} 
            className="h-8 w-8 p-0 rounded-md bg-gray-600 hover:bg-gray-500 text-white transition-all duration-200 hover:shadow-md hover:-translate-y-0.5"
          >
            <X size={18} />
          </Button>
        </CardTitle>
      ...
      <CardContent className="space-y-4 px-4 py-3 overflow-auto" style={{ maxHeight: "calc(100% - 4rem)" }}>
        ...
        <div>
          ...
          <div className="max-h-48 overflow-y-auto pr-1 space-y-2">
            {emissionsData.route && Array.isArray(emissionsData.route) ? (
              emissionsData.route.map((item, index) => {
                if (item.type === 'location') {
                  return (
                    <div key={`loc-${index}`} className="text-xs border-l-2 border-gray-500 pl-2 py-1">
                      <p className="font-medium text-white">{item.name || 'Location'}</p>
                      <p className="text-gray-400">Hub emissions: {formatNumber(item.co2e)} kg</p>
                    </div>
                  );
                  ...
                  const getModeIcon = (mode) => {
                    switch(mode) {
                      ...
                      default: return <span className="px-2 py-1 rounded bg-gray-700">{mode || 'unknown'}</span>;
                    }
                  };
                    ...
                    <div key={`leg-${index}`} className="text-xs bg-gray-700 p-2 rounded">
                      <div className="flex justify-between items-center">
                        <span className="font-medium uppercase flex items-center text-white">
                          ...
                          <span className="ml-1">{item.transport_mode || 'unknown'}</span>
                        </span>
                        <span className="text-gray-300">{formatNumber(item.distance_km)} km</span>
                      </div>
                      <div className="flex justify-between text-gray-400 mt-1">
                        <span>Emissions</span>
                        <span>{formatNumber(item.co2e)} kg</span>
                      </div>
                    </div>
                  );
                }
              ...
              <div className="text-xs bg-gray-700 p-3 rounded text-center">
                <p className="text-white">Route data unavailable</p>
              </div>
            )}
          </div>
        </div>
